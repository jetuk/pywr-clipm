...
def test_normal_matrix_vector_product(cl_context: cl.Context, cl_queue: cl.CommandQueue):
    ...
    out = normal_matrix_vector_product(cl_context, cl_queue, a, x, z, b)
...
def test_conjugate_gradient_solve(cl_context: cl.Context, cl_queue: cl.CommandQueue):
    ...
    y0 = np.zeros((n, gsize)).astype(np.float64)

    y = normal_conjugate_gradient_solve(cl_context, cl_queue, a, x, z, b, y0)
    ...
    for i in range(gsize):
        ...
        np.testing.assert_allclose(y[:, i], expected, atol=1e-6, rtol=1e-6)
...
def test_normal_equation_rhs(cl_context: cl.Context, cl_queue: cl.CommandQueue):
    ...
    rhs = normal_equation_rhs(cl_context, cl_queue, a, x, z, b, c, y, delta)
    ...
    for i in range(gsize):
        ...
        expected = -(b[:, i] - a.dot(x[:, i]) - a.dot(x[:, i] * (c[:, i] - np.array(adt.dot(y[:, i]))[0, :] + mu / x[:, i]) / z[:, i]))
